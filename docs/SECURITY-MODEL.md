...
| **Stale reads under concurrent ACL updates (TOCTOU)** | The classic Time-of-Check-to-Time-of-Use gap requires a cached permission to go stale between check and use. We never cache: each read recomputes from live source state in one synchronous pass, so the TOCTOU window does not exist by construction. Demonstrated under interleaved mutation load. |
